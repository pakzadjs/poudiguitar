...
import { removeStatement } from "@/services/adminServices";

export default function RemoveStatement({ id }) {
  ...
  const { isLoading, mutateAsync } = useMutation({
    mutationFn: removeStatement,
  });

  const removeStatementHandler = async () => {
    try {
    ...
    <>
      ...
      >
        <ModalContent className="text-slate-900 bg-sky-100/70 p-3">
          {(onClose) => (
            <>
              <ModalHeader className="text-xl font-extrabold">پاک کردن اعلامیه</ModalHeader>
              ...
              <ModalBody>
                <Button
                  ...
                  onPress={onClose}
                  onClick={removeStatementHandler}
                  isLoading={isLoading}
                >
                  پاک کردن
                </Button>
